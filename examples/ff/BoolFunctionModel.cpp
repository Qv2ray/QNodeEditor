...
std::vector<bool>
BoolFunctionModel::
applyFunction(std::vector<bool> const &range1,
              std::vector<bool> const &range2) const
{
  std::vector<bool> result;

  BoolFunctionPtr const & f =
    std::get<2>(_nameAndBoolFunctions[_functionComboBox->currentIndex()]);
...
createNameAndBoolFunctions()
{
  _nameAndBoolFunctions.push_back(
    std::make_tuple(QString("&&"),
                    QString("( %1 && %2 )"),
                    static_cast<BoolFunctionPtr>([](bool a, bool b) {
    return (a && b);
  })));
