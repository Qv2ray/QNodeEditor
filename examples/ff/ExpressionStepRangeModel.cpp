...
#include <algorithm>
ExpressionStepRangeModel::ExpressionStepRangeModel()
{
    _widget = new QWidget();
    ...
    _widget->setLayout(l);
    connect(_variableEdit, &QLineEdit::textChanged, this, &ExpressionStepRangeModel::onVariableEdited);
    connect(_numberEdit, &QLineEdit::textChanged, this, &ExpressionStepRangeModel::onVariableEdited);
    connect(_stepEdit, &QLineEdit::textChanged, this, &ExpressionStepRangeModel::onVariableEdited);
    connect(_spinBox, SIGNAL(valueChanged(QString)), this, SLOT(onVariableEdited(QString)));
}
QJsonObject ExpressionStepRangeModel::save() const
{
...
void ExpressionStepRangeModel::restore(QJsonObject const &p)
{
...
unsigned int ExpressionStepRangeModel::nPorts(PortType portType) const
{
...
std::vector<double> ExpressionStepRangeModel::processRangeText(QString const &numberText, QString const &stepText, int times) const
{
...
QString ExpressionStepRangeModel::convertRangeToText(std::vector<double> const &range) const
{
    QString result("(");
...
void ExpressionStepRangeModel::processData()
{
    bool ok = false;
    QString text = _variableEdit->text();
    QString prefix("${%1}");
    std::vector<double> range = processRangeText(_numberEdit->text(), _stepEdit->text(), _spinBox->value());
...
void ExpressionStepRangeModel::onVariableEdited(QString const &string)
{
    Q_UNUSED(string);
    processData();
}
void ExpressionStepRangeModel::onRangeEdited(QString const &string)
{
    Q_UNUSED(string);
    processData();
}
std::shared_ptr<NodeDataType> ExpressionStepRangeModel::dataType(PortType, PortIndex) const
{
    return ExpressionRangeData().type();
}
std::shared_ptr<NodeData> ExpressionStepRangeModel::outData(PortIndex)
{
    return _expression;
}
QWidget *ExpressionStepRangeModel::embeddedWidget()
{
    return _widget;
}
